...
def randomsample2d( X, n ):
    ...
    """
    arr = np.array((n,2), dtype = 'object')
    print arr.shape
    sampleix = random.sample( xrange( X.shape[0] ), int(n) )
    print sampleix

    for i in range(len(sampleix)):
    	arr[i] = X[sampleix[i]]
    	#arr[i,1] = X[sampleix[i],1]
...
if __name__ == "__main__":
	...
	#print cND_Int_mu
	mu_sigma = np.array([mu_c_int[0,0].ravel(), sig_c_int[0,0].ravel()]).T #splitcenterdata(cND_Int_mu, cND_Int_sigma)
	data = mu_sigma
	...
	# plot(centroids[:,0],centroids[:,1],'sw',markersize=8)
	# show()
